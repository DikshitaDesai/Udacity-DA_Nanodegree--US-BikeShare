...
def city_input():
    ...
    print('Enter the corresponding number below')
    print('Chicago: 1')
    print('New York: 2')
    print('Washington: 3')
    print(' ')
    city = int(input('Please choose the city for which you would like to see the Statistics: '))
    ...
    while True:
        try:
            ...
            print('\nPlease enter 1, 2 or 3 for cities\n')
    return city
...
def bike_users(df):
    print('\n* Q7. Types of users: subscribers, customers, others\n')
    return df['User Type'].value_counts()

def gender_data(df):
    try:
        print('\n* Q8. What is the breakdown of gender among users?\n')
...
def birth_years(df):
    try:
        print('\n* Q9. What is the earliest, latest, and most frequent year of birth, respectively?')
...
def main():
    ...
    month_day = month_day_info(df, time_period)

    df = time_filters(df, time_period, month, day, month_day)

    display_raw_data(df)
    
    stat_function_list = [month_freq,
     day_freq, hour_freq, 
